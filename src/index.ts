...
program
  ...
  .action(async (target: string, options: { base: string; dryRun: boolean; verbose: boolean; includeDrafts: boolean; autoPush: boolean }) => {
    try {
      // Configure logger
      const logger = configureLogger(options.verbose);
      
      // Validate GitHub CLI
      await validateGitHubCLI(logger);
      
      // Run the curation process
      await curateBranch(target, options.base, options.dryRun, options.includeDrafts, options.autoPush, logger);
    } catch (error) {
