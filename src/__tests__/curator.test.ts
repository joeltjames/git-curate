...
describe('Curator Module', () => {
  ...
  beforeEach(() => {
    ...
    (listOpenPRs as jest.MockedFunction<typeof listOpenPRs>).mockImplementation(() => Promise.resolve([]));
    (resetBranch as jest.MockedFunction<typeof resetBranch>).mockImplementation(() => Promise.resolve());
    (mergePR as jest.MockedFunction<typeof mergePR>).mockImplementation(() => Promise.resolve(true));
    (checkboxPrompt as unknown as jest.MockedFunction<typeof checkboxPrompt>).mockImplementation(mockCheckbox);
  ...
  describe('curateBranch', () => {
    ...
    it('should handle no PR selection', async () => {
      // Mock PRs list
      const mockPRs = [
        { number: 1, title: 'PR 1', headRefName: 'feature-1', isDraft: false, author: { login: 'user1' } }
      ];
    ...
    it('should merge selected PRs', async () => {
      ...
      const mockPRs = [
        { number: 1, title: 'PR 1', headRefName: 'feature-1', isDraft: false, author: { login: 'user1' } },
        { number: 2, title: 'PR 2', headRefName: 'feature-2', isDraft: false, author: { login: 'user2' } }
      ];
      (listOpenPRs as jest.MockedFunction<typeof listOpenPRs>).mockResolvedValueOnce(mockPRs);
