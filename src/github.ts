...
export async function validateGitHubCLI(logger: Logger, exec: ExecFunction = execAsync): Promise<void> {
  try {
...
export async function listOpenPRs(logger: Logger, includeDrafts: boolean = false, exec: ExecFunction = execAsync): Promise<PullRequest[]> {
  try {
    const { stdout } = await exec('gh pr list --state open --json number,title,headRefName,author,isDraft');
